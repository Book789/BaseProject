...
- (void)mh_regexContentWithWithEmojiImageFontSize:(CGFloat)fontSize{
    ...
    /// 匹配链接
    NSArray<NSTextCheckingResult *> *linkUrlResults = [[LHMomentHelper regexLinkUrl] matchesInString:self.string options:kNilOptions range:self.yy_rangeOfAll];
    for (NSTextCheckingResult *link in linkUrlResults) {
        ...
        if ([self yy_attribute:YYTextHighlightAttributeName atIndex:link.range.location] == nil) {
            ...
            // 将用户数据带出去
            highlight.userInfo = @{LHMomentLinkUrlKey:[self.string substringWithRange:link.range]};
    ...
    /// 匹配电话号码
    NSArray<NSTextCheckingResult *> *phoneResults = [[LHMomentHelper regexPhoneNumber] matchesInString:self.string options:kNilOptions range:self.string.rangeOfAll];
    ...
    for (NSTextCheckingResult *phone in phoneResults) {
        ...
        if ([self yy_attribute:YYTextHighlightAttributeName atIndex:phone.range.location] == nil) {
            ...
            YYTextHighlight *highlight = [YYTextHighlight new];
            // 将用户数据带出去
            highlight.userInfo = @{LHMomentPhoneNumberKey:[self.string substringWithRange:phone.range]};
            [highlight setBackgroundBorder:border];
