//
//  LHSegmentViewController.m
//  Project
//
//  Created by a on 2022/5/30.
//

#import "LHNestedScrollViewController.h"

@interface LHNestedScrollViewController ()<LHSegmentedViewDelegate>
...
@implementation LHNestedScrollViewController

-(void)viewWillAppear:(BOOL)animated{
    [super viewWillAppear:animated];
    
    self.segmentedView.selectedItemIndex = 0;
...
- (void)viewDidLoad {
    ...
    self.contentViewFrame = CGRectMake(0,self.segmentedView.height,
                                       screenSize.width,
                                              self.view.height - self.segmentedView.height);
    
    self.segmentedView = [[LHSegmentedView alloc] initWithFrame:CGRectMake(0, KNavigationBarHeight, kScreenWidth, 44)];
    self.segmentedView.delegate = self;
//    self.segmentedView.frame = CGRectMake(0,0,
//                                          self.segmentedView.width,
//                                          self.segmentedView.height);
...
- (void)setViewControllers:(NSArray *)viewControllers {
    ...
    for (UIViewController * controller in _viewControllers) {
        LHItem *item = [[LHItem alloc] init];
//        item.image = controller.yp_tabItemImage;
//        item.selectedImage = controller.yp_tabItemSelectedImage;
        item.title = controller.itemTitle;
//        item.attTitle = controller.yp_tabItemAttTitle;
        [items addObject:item];
    }
     self.segmentedView.items = items;
//    if (_didViewAppeared) {
//        NSLog(@"asdfasdf");
        _selectedControllerIndex = -1;
//        self.tabBar.selectedItemIndex = self.tabBar.willSelectedItemIndex;
//    }
...
- (void)setContentScrollEnabledAndTapSwitchAnimated:(BOOL)switchAnimated {
    if (!self.scrollView) {
        self.scrollView = [[LHContentScrollView alloc] initWithFrame:self.contentViewFrame];
        self.scrollView.pagingEnabled = YES;
        self.scrollView.showsHorizontalScrollIndicator = NO;
        self.scrollView.showsVerticalScrollIndicator = NO;
        self.scrollView.scrollsToTop = NO;
        self.scrollView.delegate = self.segmentedView;
        
        [self.view insertSubview:self.scrollView belowSubview:self.segmentedView];
        self.scrollView.contentSize = CGSizeMake(self.contentViewFrame.size.width * _viewControllers.count,
                                                 self.contentViewFrame.size.height);
    }
    [self updateContentViewsFrame];
...
- (void)setSelectedControllerIndex:(NSInteger)selectedControllerIndex {
    ...
    if (self.scrollView) {
        ...
        [self.view insertSubview:curController.view belowSubview:self.segmentedView];
...
- (void)segmentdView:(LHSegmentedView *)segmentdView didSelectedItemAtIndex:(NSInteger)index {
...
- (void)setItemTitle:(NSString *)itemTitle {
    objc_setAssociatedObject(self, @selector(itemTitle), itemTitle, OBJC_ASSOCIATION_COPY_NONATOMIC);
}


- (LHItem *)item {
    LHSegmentedView * segmentedView = self.nestedScrollerViewController.segmentedView;
    NSInteger index = [self.nestedScrollerViewController.viewControllers indexOfObject:self];
    return segmentedView.items[index];
}

- (LHNestedScrollViewController *)nestedScrollerViewController {
    return (LHNestedScrollViewController *)self.parentViewController;
}
