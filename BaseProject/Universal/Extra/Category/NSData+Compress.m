...
-(NSData*)compress{
    ...
    uint8_t *dstBuffer = (uint8_t *)malloc(self.length);
    memset(dstBuffer, 0, self.length);

    size_t compressResultLength = compression_encode_buffer(dstBuffer, self.length, [self bytes], self.length, NULL, COMPRESSION_LZ4);

    if (compressResultLength > 0) {
        NSData * data = [NSData dataWithBytes:dstBuffer length:compressResultLength];
        free(dstBuffer);
        return data;
    }else{
...
- (NSData*)deCompress{
    ...
    size_t decompressLen = compression_decode_buffer(dstBuffer,1024*1024*10,self.bytes,self.length,NULL,COMPRESSION_LZMA);

    if (decompressLen > 0) {
        NSData *data = [NSData dataWithBytes:dstBuffer length:decompressLen];
        free(dstBuffer);
        return data;
    }else{
