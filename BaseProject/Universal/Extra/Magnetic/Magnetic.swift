...
//            frame.origin.x -= frame.size.width / 2
    ...
    override open func addChild(_ node: SKNode) {
        ...
        if children.count % 2 == 0 {
            x = frame.width - node.frame.width-40 // right
        }
        let y = CGFloat.random(0, frame.height - node.frame.height)
...
extension Magnetic {
    ...
    open  func touchesBeganEvent(_ touches: Set<UITouch>, with event: UIEvent?) {
        guard removeNodeOnLongPress, let touch = touches.first else { return }
        touchStarted = touch.timestamp
    }
    
    open func touchesMovedEvent(_ touches: Set<UITouch>, with event: UIEvent?,location:CGPoint,previousLocation:CGPoint) {
     ...
     open func touchesEndedEvent(_ touches: Set<UITouch>, with event: UIEvent?,location:CGPoint) {
        guard let touch = touches.first else { return }
        let location = location
